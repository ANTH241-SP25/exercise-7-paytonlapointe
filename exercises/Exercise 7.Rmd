...
library(dunn.test)
```
Lets test whether age varies by gender in NHANES sample
```{r}
hist(NHANESdata$Age)
NHANESdata%>%
  drop_na(Age)%>%
...
t.test(Age~Gender, data=NHANESdata, var.equal=TRUE)
```
...
This T-test supports the alternative hypothesis.
...
#3b sednetary and health insurance
```{r}
...
NHANES_sedentary%>%
  drop_na(Sedentary)%>%
  group_by(HealthIns)%>%
  summarize(mean(Sedentary), sd(Sedentary), counts=n())
var.test(Sedentary~HealthIns, data=NHANES_sedentary, alternative="two.sided")
t.test(Sedentary~HealthIns, data=NHANES_sedentary, var.equal=FALSE, na.rm=TRUE)
```
...
Null Hypothesis:There is no difference between male and female caloric intake
Alternative: There is a difference is the caloric intake between males and females.
```{r}
NHANESdata%>%
  drop_na(DR1TKCAL)%>%
  group_by(Gender)%>%
...
t.test(DR1TKCAL~Gender, data=NHANESdata, alternative="two.sided")
```
These results shows that there is a statistically significant difference between gender when looking at calorie intake therefore supporting the alternative hypothesis.
...
#Null hypothesis is that there is no difference in caloric intake across BMI categories. The alternative hypothesis is that there is a some difference across the BMI categories
...
ggplot(NHANESdata, aes(x=BMIcat, y=DR1TKCAL))+
         ...
         labs(x="BMI category", y="calories consumed per day", title="Daily calories by BMI cat")
...
There is no statistically significant difference in caloric intake between the obese and healthy category. There is no statistically significant difference in caloric intake between the healthy and overweight category. There is no statistically significant difference in caloric intake between the obese and overweight category. There is a statistically significant difference in the daily caloric intake across the underweight across healthy, obese and overweight categories.
```{r}
...
There is a statistical significance across all categories comparing Head of household education levels compared to the ratio of family income to poverty guidelines.
...
dunn.test(x=NHANESdata$SMSpend, g=NHANESdata$Race, kw=TRUE)
```
According the Kruskal-Wallis and Dunn tests, there is a statistically significant difference in money spent on groceries per month between Mexican Americans and mixed race individuals. There is a statistically significant difference in money spent on groceries per month between Mexican Americans and non-Hispanic Asians. There is a statistically significant difference in money spent on groceries per month between Mexican Americans and non-Hispanic Blacks. There is a statistically significant difference in money spent on groceries per month between Mexican Americans and non-Hispanic whites. There is no statistically significant difference in money spent on groceries per month between Mexican Americans and individuals who identified as other. There is no statistically significant difference in money spent on groceries per month between Mexican mixed race individuals and non-Hispanic Asians.There is a statistically significant difference in money spent on groceries per month between mixed race individuals and non-Hispanic Blacks. There is a statistically significant difference in money spent on groceries per month between mixed race individuals and non-Hispanic whites. here is a statistically significant difference in money spent on groceries per month between mixed race individuals and individuals who identified as other. There is a statistically significant difference in money spent on groceries per month between non-Hispanic Asians compared across non-Hispanic Blacks, non-Hispanic whites, and individuals who identified as other. There is a statistically significant difference in money spent on groceries per month between non-Hispanic Blacks compared across non-Hispanic whites, and individuals who identified as other. in money spent on groceries per month between mixed race individuals and non-Hispanic Blacks. There is a statistically significant difference in money spent on groceries per month between individuals who identified as other and non-Hispanic whites.